...
DATA_DIR = os.path.join(ROOT, "boom_data")
OUT_DIR = ROOT
...
RENAL_IDENTITY = "#f59e0b"  # amber/gold — BOOM (biochem/molecular/genetics) identity
...
def make_html(game, prev_file, next_file, idx_in_palette):
    ...
    # Nav links
    index_link = (
        '<a href="boom_index.html" style="position:fixed;top:12px;left:12px;z-index:9999;'
...
def main():
    ...
    for i, g in enumerate(all_games):
        g["file"] = f"boom_{i+1:02d}_{g['slug']}.html"
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>BOOMStudy &ndash; Drag &amp; Drop Games</title>
<style>
...
document.getElementById('search').addEventListener('input', function() {{
  ...
  document.getElementById('noResults').style.display = anyVisible ? 'none' : 'block';
    ...
    with open(os.path.join(OUT_DIR, "boom_index.html"), "w", encoding="utf-8") as f:
        f.write(index_html)
